...
//	ivec2 coord = ivec2(gl_GlobalInvocationID.xy % imageSize(img_input));
	//vec3 col = imageLoad(img_input,coord).rgb;
		ivec2 coord = ivec2(gl_GlobalInvocationID.xy % textureSize(img_input,0));
	vec3 col = texelFetch(img_input,coord,0).rgb;

	vec4 pixel = vec4(V*col,1);
...
void Viewer::init_ogl()
{
	...
	tex_out = Texture2D::create();
	tex_out->alloc(1024,1024,GL_RGBA32F);

	tex_in = Texture2D::create();
	tex_in->load(DATA_PATH+"/chaton.png",true);
//	tex_in->alloc(4,4,GL_RGBA8);
//	GLubyte X[] =
//	{255,0,0,0, 255,0,0,0, 0,255,0,0, 0,255,0,0,
//	 255,0,0,0, 255,0,0,0, 0,255,0,0, 0,255,0,0,
//	 0,255,0,0, 0,255,0,0, 255,0,0,0, 255,0,0,0,
//	 0,255,0,0, 0,255,0,0, 255,0,0,0, 255,0,0,0 };
//	tex_in->update(0,0,4,4,X);
}
...
//	tex_in->bind_compute_in(0);
	tex_in->bind(1);
	tex_out->bind_compute_out(1);
	glDispatchCompute(GLuint(tex_out->width()/32), GLuint(tex_out->height()/32), 1);
	glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);

	glDisable(GL_DEPTH_TEST);
	prg_fs->bind();
	set_uniform_value("TU", tex_out->bind(0));
	VAO::none()->bind();
