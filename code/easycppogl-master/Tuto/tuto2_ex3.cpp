...
void createcircle(float rayon, int nb)
{
	...
	index = (unsigned int *)malloc(sizeof(unsigned int) * 2 * nb);
	for (int i = 0; i<nb; i++)
	{
		index[2 * i] = i; index[2 * i + 1] = i + 1;
	}
	ns = nb + 1;
	ni = 2 * nb;
}
...
void loadglobject()
{
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, 9 * sizeof(unsigned int), index, GL_STATIC_DRAW);
	CHECK_GL_ERROR();
...
void setshadervariables()
{
	...
	// associer le VBO à cette variable
	glBindBuffer(GL_ARRAY_BUFFER, vbo_id[2]);
